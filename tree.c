...
int increasing_length_move(Tree * input_tree, long rank) {
  if (input_tree->tree == NULL){
    ...
    return 1;
  } else if(input_tree->tree[rank].parent != -1 && input_tree->tree[rank].time + 1 == input_tree->tree[rank+1].time) {
    /*fprintf(stderr, "Error. No length move possible. length between next node is not > 1.\n");*/
    return 1;
  }
...
int decreasing_length_move(Tree * input_tree, long rank) {
  if (input_tree->tree == NULL){
    ...
    return 1;
  } else if(input_tree->tree[rank].time == 1) {
    /*fprintf(stderr, "Error. Length move not possible. Node height already 1.\n");*/
    return 1;
  } else if(input_tree->tree[rank].time - 1 == input_tree->tree[rank-1].time) {
    /*fprintf(stderr, "Error. No length move possible. length between next node is not > 1.\n");*/
    return 1;
  }
...
void print_tree_info(Tree *input_tree) {
  printf("Tree's Sum of Squared Distances: %ld\n", input_tree->sos_d);
  printf("Tree's root time: %ld\n", input_tree->root_time);
  printf("Tree's # leaves: %ld\n", input_tree->num_leaves);
  for(int i = 0; i < input_tree->num_leaves*2 -1; i++) {
    printf("Node: %d Position: %d Time: %ld\n", i, i, input_tree->tree[i].time);
...
Path findpath(Tree *start_tree, Tree *dest_tree){
    ...
    for (long i = num_leaves; i < dest_tree->root_time + num_leaves; i++){
      ...
      if(dest_tree->tree[current_node].time == i - num_leaves + 1) {
        ...
        while(current_mrca > current_node || current_mrca_time != i - num_leaves + 1){
          ...
          if (did_nni == false && did_rnkmv == false && current_tree.tree[current_mrca].time > dest_tree->tree[current_node].time) {
            ...
            if(j == current_mrca) {
              ...
              while(j >= i - num_leaves && current_tree.tree[j].time != current_tree.tree[j-1].time+1 && j >= current_node) {
                decreasing_length_move(current_tree_pointer, j);
                path.moves[path_index][1] = 4;
                path.moves[path_index][0] = j;
                j--;
  ...
  //path.moves = realloc(path.moves, (path_index) * sizeof(long*));
  return path;
...
* trees[0] = start tree, trees[1] = dest tree.
...
/* Follow path follows the path in a given path matrix by doing moves on a tree
 * up to a given index of the path.
 * @param Path * p The path to follow.
 * @param Tree * start_tree The tree to do moves on.
 * @param long path_index The number of moves to perform on the tree.
 * @return Tree The tree produced by following the path to the path index.
 */
Tree follow_path(Path * p, Tree * start_tree, long path_index) {
  //Do moves on the start_tree
  //Return the start tree.
  Tree output;
  output.tree = emalloc((start_tree->num_leaves*2 - 1) * sizeof(Node));
  copy_tree(start_tree, &output);
  for(int i = 0; i < path_index; i++) {
    if(p->moves[path_index][1] == 0) {
      rank_move(&output, p->moves[path_index][0]);
    }
    else if(p->moves[path_index][1] == 1) {
      nni_move(&output, p->moves[path_index][0], 1);
    } else if(p->moves[path_index][1] == 2) {
      nni_move(&output, p->moves[path_index][0], 0);
    } else if(p->moves[path_index][1] == 3) {
      increasing_length_move(&output, p->moves[path_index][0]);
    } else {
      decreasing_length_move(&output, p->moves[path_index][0]);
    }
  }
  return output;
}
...
/* Copy Tree does a deep copy of a tree.
* @param Tree *source_tree Tree to copy from.
* @param Tree *dest_tree TRee to copy to.
*/
int copy_tree(Tree *source_tree, Tree *dest_tree) {
  if(source_tree == NULL || dest_tree == NULL) {
    fprintf(stderr, "copy tree could not find tree.\n");
    return 1;
  }
  else {
    dest_tree->num_leaves = source_tree->num_leaves;
    dest_tree->root_time = source_tree->root_time;
    dest_tree->sos_d = source_tree->sos_d;
    for(int i = 0; i < source_tree->num_leaves*2 - 1; i++) {
      dest_tree->tree[i].parent = source_tree->tree[i].parent;
      dest_tree->tree[i].children[0] = source_tree->tree[i].children[0];
      dest_tree->tree[i].children[1] = source_tree->tree[i].children[1];
      dest_tree->tree[i].time = source_tree->tree[i].time;
    }
  }
  return 0;
}

/*One neighbourhood returns a list of trees that are one NNI or length move
* apart from the given tree.
* @param Tree t the tree to find the neighbourhood of.
* @return Tree_List the neighbourhood of the tree.
*/
Tree_List one_neighbourhood(Tree *t) {
  Tree_List tl;
  tl.num_trees = 0;
  Tree current_tree;
  current_tree.num_leaves = t->num_leaves;
  current_tree.root_time = t->root_time;
  current_tree.tree = emalloc((t->num_leaves*2 - 1) * sizeof(Node));
  /*Estimate of the maximum number of possible moves.*/
  tl.trees = emalloc(((t->num_leaves) + (t->num_leaves - 2) + (t->root_time - t->num_leaves + 1)*2) * sizeof(Tree));
  for(int i = current_tree.num_leaves; i < current_tree.num_leaves*2 - 1; i++) {
    copy_tree(t, &current_tree);
    /* A rank move cannot be performed on root+1*/
    if(i < current_tree.num_leaves*2 - 2 && rank_move(&current_tree, i) != 1) {
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      copy_tree(t, &current_tree);
    }
    /*NNI left children.*/
    if(i < current_tree.num_leaves*2 - 2 && nni_move(&current_tree, i, 0) != 1) {
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      copy_tree(t, &current_tree);
    ...
    /*NNI right children*/
    if(i < current_tree.num_leaves*2 - 2 && nni_move(&current_tree, i, 1) != 1) {
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      copy_tree(t, &current_tree);
    }
    if(increasing_length_move(&current_tree, i) != 1) {
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      copy_tree(t, &current_tree);
    }
    if(decreasing_length_move(&current_tree, i) != 1) {
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
    }
...
/* One suburb returns a list of trees that are one NNI move or multiple length
 * moves away apart from the given tree.
 * @param Tree t the tree to find the "suburb" of.
 * @return Tree_List the neighbourhood of the tree.
 */
Tree_List one_suburb(Tree *t, long max_root_time) {
  Tree_List tl;
  tl.num_trees = 0;
  Tree current_tree;
  current_tree.num_leaves = t->num_leaves;
  ...
  current_tree.tree = emalloc((t->num_leaves*2 - 1) * sizeof(Node));
  /*Estimate of the maximum number of possible moves.*/
  tl.trees = emalloc(((t->num_leaves) + (t->num_leaves - 2) + ((t->root_time - t->num_leaves + 1)*2)*3) * sizeof(Tree));
  for(int i = current_tree.num_leaves; i < current_tree.num_leaves*2 - 1; i++) {
    copy_tree(t, &current_tree);
    /* A rank move cannot be performed on root+1*/
    if(i < current_tree.num_leaves*2 - 2 && rank_move(&current_tree, i) != 1) {
    ...
    if(increasing_length_move(&current_tree, i) != 1) {
      ...
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      long moves_counter = 1;
      while(current_tree.tree[i].time < max_root_time && increasing_length_move(&current_tree, i) != 1){
        moves_counter++;
        /*printf("current %d time: %ld, max root time: %ld\n", i, current_tree.tree[i].time, max_root_time);*/
      }
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      copy_tree(t, &current_tree);
      if(moves_counter > 2) {
        for(int j = 0; j < round(moves_counter / 2); j++) {
          increasing_length_move(&current_tree, i);
        }
        tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
        copy_tree(&current_tree, &tl.trees[tl.num_trees]);
        tl.num_trees++;
    ...
    if(decreasing_length_move(&current_tree, i) != 1) {
      ...
      tl.num_trees++;
      long moves_counter = 1;
      while(decreasing_length_move(&current_tree, i) != 1) {
        moves_counter++;
        /*printf("Node %d time: %ld.\n", i, current_tree.tree[i].time);*/
      }
      tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
      copy_tree(&current_tree, &tl.trees[tl.num_trees]);
      tl.num_trees++;
      ...
      if(moves_counter > 2) {
        for(int j = 0; j < round(moves_counter / 2); j++) {
          decreasing_length_move(&current_tree, i);
        }
        tl.trees[tl.num_trees].tree = emalloc((current_tree.num_leaves*2 - 1) * sizeof(Node));
        copy_tree(&current_tree, &tl.trees[tl.num_trees]);
        tl.num_trees++;
...
/* Free Tree List frees a given Tree List.
